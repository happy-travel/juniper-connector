...
public class AccommodationUpdater : IUpdateWorker
{
    ...
    private Accommodation Convert(Hotel hotel, List<Zone> zones)
    {
        ...
        var (country, locality) = GetZone(data.Zone.Code, zones);
        ...
        static (string, string) GetZone(string zoneCode, List<Zone> zones)
        {
            Zone zone;
            ...
            while (zoneCode != null)
            {
                ...
                if (zone.AreaType == ZoneType.PAS)
                    country = zone.Name;

                if (zone.AreaType == ZoneType.CTY || zone.AreaType == ZoneType.LOC)
                    locality = zone.Name;
            }
